...
if __name__ == "__main__":
    ...
    _res = stanford_business.join(review).map(
        lambda x:x[1][1]).distinct().join(user).distinct().map(lambda x: (x[1][1],x[1][0]))
    
    res = _res.collect()
    
    sc.parallelize(res).coalesce(1).saveAsTextFile("/FileStore/shared_uploads/pxj200018@utdallas.edu/Q3_HW")